...
exports.updateDeviceById = (req, res, next) => {
    ...
    DeviceInfoDB
        .findOneAndUpdate({hardwareID: data.hardwareID}, data.updateInfo, {new: true})
        .then((value)=>{
            return res.status(200).json({statusCode: 'success', param: value});
        })
        .catch((err)=>{
            return res.status(404).json({statusCode: 'fail', err: err});
        })
