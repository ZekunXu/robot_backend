    ...
    Promise
        ...
        .then(()=>{
            ...
            if(data.msgType && !data.startDate && !data.endDate){
                var param = {msgType: data.msgType};
            }else if(!data.msgType && data.startDate && data.endDate){
                var param = {timeStamp: {$gte: data.startDate, $lte: data.endDate}};
            }else if(data.msgType && data.startDate && data.endDate){
                var param = {msgType: data.msgType, timeStamp: {$gte: data.startDate, $lte: data.endDate}};
            }

            return HardwareMsgDB
                    .find(param ? param : null) //如果有消息类型，则按照消息类型查询，如果没有则返回全部
